...
public:
  ...
  {
    sim_only = get_parameter("sim_only").as_bool();
    //if(sim_only_val.compare("true") == 0){
    //	sim_only = true;
    //}else{
    //	sim_only = false;
    //}
    // Create the gripper service callback. This will be called when the 
    auto fake_gripper_service = [this](
      ...
      void {
        ...
        if (configuration_type == 1) {
          ...
          if(!sim_only){
            send_request_to_hardware(request->rpr);
          }
...
private:
  ...
  void send_request_to_hardware(
    int gripper_width){
      rclcpp::Client<robotiq_msgs::srv::CModelResponse>::SharedPtr client =
        create_client<robotiq_msgs::srv::CModelResponse>(get_parameter("hardware_ros_service").as_string());
      ...
      if (!srv_future_.valid()) {
        RCLCPP_INFO(get_logger(), "Client Not started");
        srv_future_ = client->async_send_request(request);

        while (srv_future_.wait_for(std::chrono::nanoseconds(0)) ==
            std::future_status::timeout)
        {
            // RCLCPP_INFO(node_->get_logger(), "Access Request still Ongoing");
        }
        RCLCPP_INFO(get_logger(), "Received result");
        auto srv_result = srv_future_.get();
        srv_future_ = {};
    } else {
        RCLCPP_ERROR(get_logger(), "Throw Error: Future ERROR");
    }
  }

  std::shared_future<rclcpp::Client<robotiq_msgs::srv::CModelResponse>::SharedResponse> srv_future_;
};
}  // namespace gripper_service
