...
class ArucoNode(rclpy.node.Node):
    def __init__(self):
        ...
        qos_profile = QoSProfile(
            ...
            depth=10  # Set an appropriate depth based on your message frequency
        )
        ...
        self.recorded_poses = None
        self.recorded_markers = None
    ...
    # Callback to retrieve pose information
    def spawn_collision_obj_callback(self, request, response):
        response.detect_id = False

        if request.marker_id in self.recorded_markers:
            response.detect_id = True
            idx = self.recorded_markers.index(request.marker_id)
            response.obj_pose = self.recorded_poses[idx]
    ...
    def image_callback(self, img_msg):
        ...
        if marker_ids is not None:
            ...
            self.recorded_markers = markers.marker_ids
            self.recorded_poses = pose_array.poses
            self.poses_pub.publish(pose_array)
            self.markers_pub.publish(markers)
        else:
            self.recorded_markers = [-1]
            self.recorded_poses = None
